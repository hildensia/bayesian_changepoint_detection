...
`tests/conftest.py` fails collection when one is missing or both are
present:

- `math`: the value is checked against something independent of this
  code base: a `scipy` evaluation, an exhaustive enumeration of
  segmentations, a closed form from a cited paper, a second derivation
  (the offline marginal against the product of online predictives). These
  are the tests that fail when the mathematics is wrong.
- `behaviour`: the test pins how the library behaves today: API contracts,
  input validation, device handling, detection on synthetic data with known
