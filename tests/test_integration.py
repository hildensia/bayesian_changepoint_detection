...
class TestIntegration:
    ...
    def test_performance_scaling(self):
        ...
        assert (
            times[-1] < times[0] * 20
        )  # Should not be more than 20x slower for 4x data
...
class TestRegressionAgainstOriginal:
    ...
    def test_simple_detection_regression(self):
        ...
        hazard_func = partial(
            constant_hazard, 50
        )  # More frequent changepoints expected
        ...
        detected_near_100 = any(
            95 <= idx <= 105 for idx in detected + high_prob_indices
        )
