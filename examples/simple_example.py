...
def run_online_detection(data):
    ...
    print("✓ Online detection completed")
    print(f"  Segment starts on the MAP path: {get_map_changepoints(R).tolist()}")
    print(
        f"  Max lag-{lag} changepoint probability (t > 0): "
...
def run_offline_detection(data):
    ...
    print("✓ Offline detection completed")
...
def main():
    ...
    try:
        plot_results(
            data, online_changepoint_probs, offline_changepoint_probs, true_changepoints
        )
    except ImportError:
