...
def main():
    ...
    for i in range(len(changepoint_probs_offline)):
        if i == 0 or i == len(changepoint_probs_offline) - 1:
            continue
        if (
            changepoint_probs_offline[i] > changepoint_probs_offline[i - 1]
            and changepoint_probs_offline[i] > changepoint_probs_offline[i + 1]
            and changepoint_probs_offline[i] > 0.01
        ):
    ...
    try:
        ...
        axes[1].plot(
            changepoint_probs.cpu().numpy(), "g-", linewidth=2
        )  # lag-10 P(change at t)
